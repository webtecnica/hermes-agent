...
def init_agent(
    ...
    # Serialize close-flush and turn-setup persistence to prevent duplicating
    # the staged CLI user message when terminal close races with turn start
    # (issue #63766). Acquired by both _persist_active_session_before_close()
    # and the crash-resilience persist in build_turn_context.
    agent._persist_lock = threading.RLock()
    # Set True by build_turn_context under _persist_lock so the close path
    # knows turn setup is already running and can skip its flush.
    agent._turn_active = False
    # Populated by the close path with the staged user content so turn setup
    # can transfer the _db_persisted marker to the new agent-owned user dict.
    ...
    agent._skill_view_called = False
    if not skip_memory:
        try:
    ...
    # Freeze environment hints toggle.  Default False.  When True, the date
    # in the volatile system-prompt tier and the CWD in the environment-hints
    # block are replaced with static placeholders so the stable+context prefix
    # is byte-identical across sessions.  This enables cross-session prefix
    # caching on providers (e.g. DeepSeek) whose server-side KV cache is
    # keyed by the prompt's leading tokens.  The agent can still discover the
    # real date and path via tools (`date`, `pwd`).
    agent._freeze_environment_hints = bool(_agent_section.get("freeze_environment_hints", False))
