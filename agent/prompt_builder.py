...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "load it with skill_view(name) and follow its instructions — err on the side of "
            "loading: skills have tool commands, API endpoints, and proven workflows that "
            "outperform general-purpose tools. They also encode preferred approaches, "
            "conventions, and quality standards for code review, planning, and testing.\n"
            "For Hermes itself (CLI, config, models, providers, tools, skills, voice, "
            "gateway, plugins, any feature) load the `hermes-agent` skill first — it has "
            "the actual commands so you don't guess workarounds.\n"
            "Fix skill issues with skill_manage(action='patch'). After hard tasks, "
            "offer to save as a skill; update skills that were missing steps or wrong commands.\n"
            "\n"
            "<available_skills>\n"
            + "\n".join(index_lines) + "\n"
            "</available_skills>\n"
            "\n"
            "Only proceed if none are relevant."
            + hidden_note
        )
