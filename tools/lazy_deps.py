...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    "tool.dashboard": (
        ...
        "starlette==1.3.1",  # CVE-2026-48710 (BadHost) + post-patch hardening — keep lazy-install in sync with pyproject [web]
        "python-multipart==0.0.31",  # FastAPI UploadFile/Form for streaming uploads (NS-501) — multipart hardening
    ),
    ...
    "tool.computer_use": (
        "mcp==1.28.1",
        "starlette==1.3.1",  # CVE-2026-48710 + post-patch hardening — keep in sync with pyproject [computer-use]
    ),
